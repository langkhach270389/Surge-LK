...
const eva = $request;
const ipaUrl = eva.url.match(/\/jsbox/) ? "http://localhost:8080/download?path=" : "http://localhost/";
//%2Fapp.ipa
...
<dict>
		...
		<dict>
			...
			<array>
				<dict>
					...
					<key>url</key>
					<string>https://langkhach/download${eva.url.match(/jsbox/)?"/jsbox":""}</string>
				</dict>
			</array>
			<key>metadata</key>
			<dict>
			...
			$notification.post('APP installation failed', '', 'Unable to read IPA installation package');
			$done()
		}
	})
	...
	$httpClient.head(ipaUrl, (err, resp, data) => {
		if (resp && resp.headers && resp.status == 200) {
			...
			const size = `Application size: ${(resp.headers['Content-Length'] / 1000 / 1000).toFixed(2)} MB`
			$notification.post(decodeURIComponent(name), size, '');
		} else {
			$notification.post('APP installation failed', '', `Unable to download the IPA installation package`);
		}
		$done({
			url: ipaUrl
		});
