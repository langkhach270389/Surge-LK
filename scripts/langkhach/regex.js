...
@langkhach
*/
const $ = API("GenUnlock", true);

const port = `6154`;
const key = `langkhach`;
const api = {
  url: `http://127.0.0.1:${port}/v1/profiles/current?sensitive=0`,
  headers: {
    "X-Key": `${key}`
  }
};
...
[Script]
Unlock.apps = type=http-response,pattern=^https?://${regex},requires-body=1,max-size=0,script-path=scripts/langkhach/unlock.apps.js,script-update-interval=-1`;
}

function domain2regex(string) {
