...
$.http.get(request).then(resp => {
  ...
  let difference = arrdomain.filter(x => !arrwhite.includes(x));
  $.log(difference);
  $.done({
    response: {
      status: 200,
...
function url2domain(url){var hostname;if(url.indexOf("//")>-1){hostname=url.split('/')[2];}
